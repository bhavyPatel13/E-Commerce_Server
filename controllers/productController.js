    ...
    if (req.files && req.files.images) {
        ...
        for (const image of images) {
            const result = await cloudinary.uploader.upload(image.tempFilePath, {
                folder: "Ecom_store_Product_Images",
                width: 1000,
                crop: "scale"
            });
...
export const fetchAllProducts = catchAsyncError(async (req, res, next) => {
    ...
    if (availability === "in-stock") {
        ...
        conditions.push(`stock = 0`); // Fixed typo: "stoke" -> "stock"
    }

    // filter product by price
    if (price) {
        const [minPrice, maxPrice] = price.split("-"); // Fixed typo: "spilt" -> "split"
    ...
    const newProductsQuery = `
        ...
        FROM products p
        LEFT JOIN reviews r ON p.id = r.product_id
        WHERE p.created_at >= NOW() - INTERVAL '38 days'
        GROUP BY p.id
        ORDER BY p.created_at DESC
        LIMIT 8
    `; // Fixed: added "-" operator, fixed JOIN syntax, removed trailing comma in GROUP BY
    ...
    const topRatedQuery = `
        ...
        LIMIT 8
    `; // Fixed: fixed JOIN syntax, removed trailing comma in GROUP BY
...
export const updateProduct = catchAsyncError(async (req, res, next) => {
    ...
    // Fixed: Added the missing comma right after the template literal string
    const result = await database.query(
        `UPDATE products SET name = $1, description = $2, price = $3, category = $4, stock = $5 WHERE id = $6 RETURNING *`,
        [name, description, price / 95.59, category, stock, productId]
    );
    res.status(200).json({
        success: true,
...
export const postProductReview = catchAsyncError(async (req, res, next) => {
    ...
    const isAlreadyReviews = await database.query(`
...
export const deleteReview = catchAsyncError(async (req, res, next) => {
    ...
    res.status(200).json({
        success: true,
        message: "Youe Review has been deleted.",
        review: review.rows[0],
        product: updateProduct.rows[0]
    })
})